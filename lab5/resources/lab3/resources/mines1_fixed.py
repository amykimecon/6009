...
def new_game(num_rows, num_cols, bombs):
    ...
    for bx, by in bombs: #This is not robust against using tuples/lists
        board[bx][by] = '.' #Should be board[bx][by], also see above for why this wouldn't work
    for x in range(num_rows):
        for y in range(num_cols):
            if board[x][y] == 0: #Should be board[x][y]
                for nx, ny in neighbors([num_rows, num_cols], x, y):
                    if board[nx][ny] == '.': #Should be board[nx][ny]
                        board[x][y] += 1 #Should be board[x][y]
