...
def dig(game, row, col):
    ...
    count = 1
    for nr, nc in neighbors(game['dimensions'], row, col): #reveals neighbor even if its a bomb
        count += 1
    ...
    game['state'] = status
    return count
#No reveal_squares, so it doesn't recursively check each neighboring square
