...
def getNames(idNumSet):
    filename = 'resources/names.json'
    with open(filename, 'r') as n:
        names = json.load(n)
        nameSet = {}
        for name, idNum in names.items():
            nameSet[idNum] = name
        resultNames = set()
        for actorid in idNumSet:
            resultNames.add(nameSet[actorid])
        return resultNames

def getNamesList(idNumList):
    filename = 'resources/names.json'
    with open(filename, 'r') as n:
        names = json.load(n)
        nameSet = {}
        for name, idNum in names.items():
            nameSet[idNum] = name
        resultNames = []
        for actorid in idNumList:
            resultNames.append(nameSet[actorid])
        return resultNames
...
def getMovies(idMovies):
    ...
    with open(filename, 'r') as n:
        movies = json.load(n)
        movieDict = {}
        for movie, idNum in movies.items():
            movieDict[idNum] = movie
        resultMovies = []
        for movieID in idMovies:
            resultMovies.append(movieDict[movieID])
        return resultMovies
...
class TestTiny(unittest.TestCase):
    ...
    def test_any_path_01(self):
        actor_id_1 = 1532
        actor_id_2 = 46866
        expected = [1532, 4724, 2876,1640,46866]
...
class TestActedTogether(unittest.TestCase):
    ...
    def test_other(self):
        actor1 = getID("Francois Perier")
        actor2 = getID("Noam Jenkins")
        print(lab.did_x_and_y_act_together(self.data, actor1, actor2))
...
class TestBaconNumber(unittest.TestCase):
    ...
    def testNamesSmall(self):
        result3 = lab.get_actors_with_bacon_number(self.data, 3)
        result4 = lab.get_actors_with_bacon_number(self.data, 4)

        print(getNames(result3))
        #print(result4)

    def testNamesLarge(self):
        result5 = lab.get_actors_with_bacon_number(self.data, 5)
        result6 = lab.get_actors_with_bacon_number(self.data, 6)

        f = open("results.txt", "w+")
        f.write(str(getNames(result6)))
        f.close()
...
class TestBaconPath(unittest.TestCase):
    ...
    def test_other(self):
        actor_id = getID("Vjeran Tin Turk")
        result = lab.get_bacon_path(self.db_large, actor_id)
        print(getNamesList(result))

class TestAnyPath(unittest.TestCase):
    def setUp(self):
        """ Load actor/movie database """
        with open('resources/small.json', 'r') as f:
            self.db_small = json.load(f)
    ...
    def test_12(self):
        result = lab.get_path(self.db_large,getID("John Hurt"),getID("Sven Batinic"))
        print(getNamesList(result))

    def test_13(self):
        result = lab.get_path(self.db_large,getID("Jihan Raja Lawak"),getID("Jim Meskimen"))
        print(getNamesList(result))
...
class TestMoviePath(unittest.TestCase):
    ...
    def test_14(self):
        result = lab.get_movies(self.db_large, getID("Iva Ilakovac"),getID("Mary Louise Weller"))
        print(getMovies(result))

    def test_15(self):
        result = lab.get_movies(self.db_large, getID("Meryl Streep"),getID("Guo Guo"))
        print(getMovies(result))
